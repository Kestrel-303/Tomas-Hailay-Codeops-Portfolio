...
AccountRegistory = AccountRegistory
...
def bfs(transfers, start):
    ...
    while queue:
        ...
        for recipient in transfers.get(curr, []):
            if recipient not in visited_set:
                visited_set.add(recipient)
...
def main():
    print("=== Addis Bank - Day 09 Mini Project: Bank Model ===\n")
    ...
    acc3 = AccountRegistory.create("savings", "Tigist Assefa", "SAV-002", 1500)
    acc4 = AccountRegistory.create("checking", "Meron Tadesse", "CHK-002", 2000)
    ...
    top_accounts = AccountRegistory.top_by_balance([acc1, acc2, acc3, acc4])
    ...
    region_central = Branch("Central Region")
    region_south = Branch("South Region")
    
    branch_bole = Branch("Bole Branch")
    branch_piazza = Branch("Piazza Branch")
    branch_hawassa = Branch("Hawassa Branch")

    head_office.add_child(region_central)
    head_office.add_child(region_south)
    
    region_central.add_child(branch_bole)
    region_central.add_child(branch_piazza)
    region_south.add_child(branch_hawassa)

    branch_bole.add_account(acc1)
    branch_piazza.add_account(acc2)
    branch_hawassa.add_account(acc3)
    head_office.add_account(acc4)

    print(f"Total Balance across Head Office Tree: {head_office.total_balance()} {config.currency}")
    print(f"Total Balance for Central Region Subtree: {region_central.total_balance()} {config.currency}")
    print(f"Total Balance for Bole Branch: {branch_bole.total_balance()} {config.currency}")

    print("\n=== Transfers Graph & BFS Traversal ===")
    transfers = {
        "SAV-001": ["CHK-001", "SAV-002"],
        "CHK-001": ["CHK-002"],
        "SAV-002": ["CHK-002"],
        "CHK-002": []
    }
    
    start_account = "SAV-001"
    reachable = bfs(transfers, start_account)
    print(f"Accounts reachable from {start_account} via transfers (BFS): {reachable}")
